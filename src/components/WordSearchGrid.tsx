...
export function WordSearchGrid({
  ...
  const cellSize = gridSize <= 10 ? 'w-8 h-8 text-base sm:w-10 sm:h-10 sm:text-lg' 
          ...
          row.map((letter, colIndex) => {
            ...
            return (
              <div
                ...
                className={cn(
                  cellSize,
                  'flex items-center justify-center font-medium rounded-md cursor-pointer transition-all duration-200',
                  'bg-background/60 text-foreground',
                  isFound && 'cell-found font-semibold',
                  isHint && !isFound && 'cell-hint',
                  isSelected && !isFound && !isHint && 'cell-selected',
                  !isFound && !isSelected && !isHint && 'hover:bg-muted'
                )}
              >
                {letter}
              </div>
